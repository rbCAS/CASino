    ...
    context 'with a valid login ticket' do
      ...
      context 'with valid credentials' do
        context 'without a service' do
          it 'should call the #user_logged_in_without_service method on the listener' do
            listener.should_receive(:user_logged_in_without_service).with(no_args)
            processor.process(lt: login_ticket.ticket, username: 'testuser', password: 'foobar123')
          end
        end

        context 'with a service' do
          it 'should call the #redirect_to method on the listener' do
            service = 'https://www.example.com/'
            listener.should_receive(:redirect_to).with(service)
